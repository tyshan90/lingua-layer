...
For Gemini API or AI Studio, pass the same contents as `system_instruction`. If the application can execute custom functions, map the state adapter operations (`status`, `setup`, `switch`, `configure`, `add-word`, `consume`, `progress-check`, `progress-response`, `pause`, and `resume`) to functions backed by a per-user state store.
