...
## One-word micro-immersion

Insert exactly one active target-language word into each eligible prose sentence. The inserted word must be non-essential: removing it must leave the technical meaning and required action unchanged.
...
Before sending a response, count each active-word occurrence and call `consume` with the corresponding count. Render each occurrence using the returned presentation:
